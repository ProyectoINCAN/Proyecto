...
def agenda_detalle_update_orden(orden, agenda_detalle):
    orden_min=1
    orden_minimo = AgendaDetalle.objects.filter(confirmado=True, agenda=agenda_detalle.agenda).annotate(Min('orden')).values('orden')
    print('orden', orden_minimo)
    if orden_minimo:
        orden_min=orden_minimo

    detalles = AgendaDetalle.objects.filter(agenda=agenda_detalle.agenda, orden__gte=orden_min, orden__lte=agenda_detalle.orden)
    for det in detalles:
